...
## Virtual scroll del catálogo (CDK) — escalabilidad a miles de productos (2026-07-22)

El grid del catálogo usa **`@angular/cdk` Virtual Scroll** (`cdk-virtual-scroll-viewport`). Solo las filas
visibles existen en el DOM; al hacer scroll, CDK **recicla** esas filas en vez de crear miles de cards.

### Problema que resolvió

Antes, el `@for (item of itemsCatalogo())` montaba **una card en el DOM por producto**. `content-visibility: auto`
evitaba *pintarlas*, pero los N nodos igual se **creaban** en cada cambio de filtro. Con cientos de productos el lag
era imperceptible; a miles (1k–15k, el objetivo SaaS multi-tenant), cambiar a "Todos" reconstruía N nodos de golpe
→ **micro-lag perceptible al filtrar**. La virtualización lo elimina de raíz: cambiar de filtro pasa de
`O(N productos)` a `O(filas visibles)` — igual de rápido con 500 que con 15.000.

> ⚠️ Esto ataca **solo** el cuello de botella del render del grid. Los otros dos que escalan linealmente con el
> catálogo (firmado de N signed URLs por carga, y binarios de imagen a disco) siguen pendientes — anotados como
> optimización futura en `PLAN-OFFLINE-POS-2026-06-08.md` §13.4 (firmar bajo demanda / miniaturas reales al subir).

### Cómo funciona (chunking por filas)

CDK virtualiza listas de **1 columna**, pero el catálogo es un grid de N columnas. Se resuelve agrupando los
`CatalogoItem` en **filas** de `columnasCatalogo` elementos y virtualizando esas filas:

| Pieza (`pos.page.ts`) | Rol |
|---|---|
| `filasCatalogo` (computed) | Agrupa `itemsCatalogo()` en chunks de `columnasCatalogo()` — cada chunk es una fila del grid |
| `columnasCatalogo` (signal) | Nº de columnas según viewport. Se pasa al CSS vía `[style.--cols]` para que chunking (TS) y layout (CSS) **siempre coincidan** — con `auto-fill` el CSS podría poner un nº distinto al agrupado → filas con huecos |
| `alturaFilaCatalogo` (signal) | `itemSize` del viewport = ancho de columna × 4/3 (aspect-ratio 3/4 de la card) + gap. Debe ser **preciso** o el scroll se desincroniza (filas se solapan / dejan hueco) |
| `calcularColumnas()` / `calcularAlturaFila()` | Reflejan el `grid-template-columns` del SCSS: mobile auto (~110px/col), tablet 5, desktop 4, ≥1440px 5. El gap difiere (12px desktop, 10px mobile) |
| `onResizeCatalogo()` (`@HostListener('window:resize')`) | Recalcula columnas + alto y llama `checkViewportSize()` al rotar/redimensionar |
| `trackFila` | trackBy por índice de fila — re-publicar imágenes progresivas actualiza el contenido de la fila sin recrear su DOM |

Template: `<cdk-virtual-scroll-viewport #catalogoViewport class="catalogo-vscroll" [itemSize]="alturaFilaCatalogo()"
[style.--cols]="columnasCatalogo()">` con `*cdkVirtualFor="let fila of filasCatalogo(); trackBy: trackFila"`; dentro,
el mismo `@for` de cards de siempre. **Las cards no cambiaron** — solo se envolvieron en `.catalogo-fila` (grid de `--cols`).

### Arquitectura del scroll — por qué NO se usó `[scrollY]="false"`

El patrón oficial de Ionic para CDK dentro de `ion-content` es `[scrollY]="false"` + clase `ion-content-scroll-host`.
**No se usó** porque: (a) afectaría a TODO el `ion-content` (compartido con la vista lista), y (b) tiene bugs
conocidos de `ion-refresher` disparándose mal con CDK (issues Ionic #25318/#25495/#26553).

En su lugar, el viewport es un **scroller independiente con altura acotada**: `.catalogo-wrapper` es `height: 100%`
(no `min-height`) → la barra de categorías queda **fija arriba** y el viewport toma el alto restante y scrollea
internamente. `ion-content` no scrollea el catálogo, así que **sus `slot="fixed"` (pill "Ver carrito", FAB subir,
botón escáner, preview de escaneo) siguen intactos** — no hubo que reposicionarlos. La cadena de altura funciona a
través de `.pos-layout`/`.pos-col-main` aunque sean `display: contents` (verificado en el motor de layout).

**Desktop:** `.pos-col-main` cede su `overflow-y` al viewport (sin esto habría doble scroll). El FAB subir ya estaba
oculto en desktop, sin cambios ahí.

### Ciclo de vida — recalcular el viewport

El viewport CDK queda con **altura 0** si se renderiza oculto o se recrea. Se llama `checkViewportSize()` en:
`ionViewDidEnter` (página cacheada por Ionic al volver), `volverAlCatalogo()` (mobile recrea el viewport al alternar
lista↔catálogo), y `onResizeCatalogo()`. Al cambiar de categoría/filtro se hace `scrollToOffset(0, 'auto')` para
arrancar desde arriba.

### Cambios de UX asumidos (decididos 2026-07-22)

| Antes | Ahora | Motivo |
|---|---|---|
| Barra de categorías scrolleaba con el grid | **Fija arriba** (sticky), solo el grid scrollea | Mejor UX (filtros siempre a mano, estándar Loyverse/Square) + requisito de la virtualización |
| Pull-to-refresh en el catálogo | **Solo en la vista lista** | El catálogo se refresca solo (al entrar al POS, tras cada venta, descuento local en vivo). El `ion-refresher` requiere que `ion-content` scrollee, cosa que el catálogo ya no hace |
| `content-visibility: auto` en `.catalogo-card` | Eliminado | Redundante con virtual scroll (las cards fuera de pantalla ni existen); juntos podían dar mediciones de alto inconsistentes |

### Interacciones verificadas intactas

Fly-to-pill (el clon se genera **antes** del reciclaje; el pill/panel destino viven fuera del viewport), long-press
de favoritos (`touch-action: pan-y` + `pointercancel` sigue igual; el viewport es el scroller), escáner, badges de
stock, imágenes progresivas (re-publicar `productosCatalogo` recomputa `filasCatalogo` sin recrear cards).
...
| `filasCatalogo` | `computed<CatalogoItem[][]>()` | Agrupa `itemsCatalogo()` en filas de `columnasCatalogo()` para el virtual scroll (ver "Virtual scroll del catálogo") |
| `columnasCatalogo` | `signal(number)` | Columnas del grid según viewport (sincroniza chunking TS ↔ layout CSS vía `--cols`) |
| `alturaFilaCatalogo` | `signal(number)` | `itemSize` del viewport CDK (alto de fila en px) |
...
- `@angular/cdk/scrolling` (`ScrollingModule`) — virtual scroll del grid del catálogo (ver "Virtual scroll del catálogo")
